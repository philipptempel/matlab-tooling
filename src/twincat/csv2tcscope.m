...
%     names like 'Global.axes[0].ActPos' would not be handled well
%   2018-02-25
...
nDateFormats = numel(ceDateFormats);
...
for iHeader = 1:nLines_Header
  ...
  switch lower(ceLine{1})
    ...
    case 'startrecord'
      ...
      for ifmt = 1:nDateFormats
        try
          [stSignalData(:).StartRecord_DateTime] = deal(datetime(sprintf('%s %s', ceLine{3}, ceLine{4}), 'InputFormat', ceDateFormats{ifmt}));
          [stSignalData(:).StartRecord_Timestamp] = deal(ceLine{2});

          % Found one, break out of our foor loop
          break
        catch me
        end
      end
    ...
    case 'endrecord'
      ...
      for ifmt = 1:nDateFormats
        try
          [stSignalData(:).EndRecord_DateTime] = deal(datetime(sprintf('%s %s', ceLine{3}, ceLine{4}), 'InputFormat', ceDateFormats{ifmt}));
...
if chCollectSignals == matlab.lang.OnOffSwitchState.on
  ...
  for iSymbol = 1:nUniqueSignals
    ...
    stSignalCombinations(iSymbol).Signals = find(~cellfun(@isempty, regexp({stSignalData.SymbolName}, [regexptranslate('escape', ceUniqueSymbolNames{iSymbol}) , '(\[\d+\])?$'])));
  end
...
for iVar = 1:nSignals
  ...
  tcsSignal.File = stSignalData(iVar).File;
